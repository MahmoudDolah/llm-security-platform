...
try:
    REQUEST_COUNT = Counter(
        "llm_requests_total", "Total number of LLM requests", ["status", "backend"]
    )
    REQUEST_BLOCKED = Counter(
        "llm_requests_blocked", "Number of blocked requests", ["reason"]
    )
...
async def chat(
    ...
    if len(request.prompt) > config.security.max_prompt_length:
        ...
        raise HTTPException(
            ...
            detail=(
                ...
                f"{config.security.max_prompt_length} characters"
            ),
        )
    ...
    try:
        ...
        REQUEST_COUNT.labels(status="error", backend=config.llm.backend).inc()

        logger.error("LLM request failed", user=user_info["name"], error=str(e))
...
async def root():
    ...
    return {
        ...
        "version": config.app_version,
        "description": "Production-ready security gateway for LLM applications",
        "endpoints": {"chat": "/v1/chat", "health": "/health", "metrics": "/metrics"},
        "security_features": [
