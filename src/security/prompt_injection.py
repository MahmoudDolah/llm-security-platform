...
class PromptInjectionDetector:
    ...
    INJECTION_PATTERNS = {
        ...
        "system_prompt_leak": [
            (
                r"(show|display|print|reveal|tell)\s+(me\s+)?(your|the)\s+"
    ...
    def detect(self, prompt: str) -> DetectionResult:
        ...
        keyword_count = sum(
            1 for keyword in self.HIGH_RISK_KEYWORDS if keyword in prompt.lower()
        )
    ...
    def _calculate_confidence(
        self, pattern_scores: List[float], keyword_count: int, prompt_length: int
    ) -> float:
        """
    ...
    def _determine_risk_level(
        self, confidence: float, detected_patterns: List[str]
    ) -> str:
